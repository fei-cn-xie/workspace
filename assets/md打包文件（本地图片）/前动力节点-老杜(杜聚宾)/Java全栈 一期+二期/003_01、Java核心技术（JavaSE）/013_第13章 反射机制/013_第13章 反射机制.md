...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/DOyPbwu9NoQ5tPx3yVfcYbo5nUf.jpeg)
...
![60007bfc-984d-45a6-9e43-84133f83e748.png](images/AvlsbdsFCoFMQZxksKNcmXlNnXe.png)

## 反射的优缺点

### 优点

1. 提高了Java程序的灵活性和扩展性，降低了耦合性，提高了自适应能力。
2. 允许程序创建和控制任何类的对象，无需提前硬编码目标类。

### 缺点

1. 反射的性能较低，反射主要应用在对灵活性和扩展性要求高的系统框架上。
2. 反射会模糊程序内部的逻辑，从而造成程序的可读性比较差。
3. 突破封装的限制，即使私有的内容也可以通过反射进行操作（优点|缺点）。
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/AfZBbR7tqo7ZOtxQcfmcKv81nXe.jpeg)
...
已知某个类的实例，还可以调用该实例的getClass()方法获取Class对象。
...
已知一个类的全类名，还可通过Class类的静态方法forName(String className)获取，但是可能会抛出ClassNotFoundException异常。

```java
// 通过全类名来获得Class对象
Class<?> clazz = Class.forName("com.jkweilai.reflect.Tiger");
System.out.println(clazz);
```

### 方式四（了解）

通过类加载器来获得Class对象，以下代码中使用的是系统类加载器（虚拟机提供）。

```java
// 获得系统类加载器
ClassLoader classLoader = ClassLoader.getSystemClassLoader();
// 通过类加载器来获得Class对象
Class<?> clazz = classLoader.loadClass("com.jkweilai.reflect.Tiger");
System.out.println(clazz);
```

## Class可以指哪些结构

在Java语言中，Class可以指任意Java类型。通过class属性，我们可以获得基本数据类型、void、数组、枚举、接口、泛型和类等类型的Class对象。
...
对于Tiger类而言，Tiger类的Class对象只有一份，因为类只会加载一次。因此，以下四种方式获得Tiger类的Class对象，获得的Class对象的哈希值一模一样。

```java
// 1.通过class属性来获得
Class<Tiger> clazz01 = Tiger.class;
System.out.println(clazz01.hashCode()); // 输出：1163157884

// 2.通过对象的getClass()方法获得
Tiger tiger = new Tiger();
Class<? extends Tiger> clazz02 = tiger.getClass();
System.out.println(clazz02.hashCode()); // 输出：1163157884

// 3.通过forName()方法获得
Class<?> clazz03 = Class.forName("com.jkweilai.reflection.Tiger");
System.out.println(clazz03.hashCode()); // 输出：1163157884

// 4.通过类加载器来获取
// 获得类加载器
ClassLoader classLoader = ClassLoader.getSystemClassLoader();
// 通过类加载器获得Class对象
Class<?> clazz04 = classLoader.loadClass("com.jkweilai.reflection.Tiger");
System.out.println(clazz04.hashCode()); // 输出：1163157884
```

![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/O4qjbS5OAohK9exRH4CcXZThnSh.jpeg)

# 类的加载过程分析

## 加载类的分析

字节码文件需要加载到虚拟机中之后才能运行和使用，那么虚拟机是如何加载这些字节码文件呢？？？系统加载字节码文件主要有三步：装载 -> 链接 -> 初始化。

![3ba69226-00ae-4b02-aaf1-e480cacf0858.png](images/FLQLbBBrfoTK76xHspzcpBSPnyc.png)
...
确保加载类的信息符合JVM规范，例如：以0xcafebabe开头，就没有安全问题（**在 IDEA 中可以安装 Hex Viewer 插件查看魔数**）。
...
符号引用：我要调用 Math 类的 abs()方法。

直接引用：直接通过 Math.abs()方法的地址定位到方法，不再找了。

### 初始化（initialization）

在类加载的**初始化阶段**，**JVM 会执行**编译期生成的 `<font style="color:rgb(15, 17, 21);background-color:rgb(235, 238, 242);"><clinit>()</font>` 方法。（静态变量显示初始化赋值+静态代码块执行）

该方法由编译器自动合成，包含了**所有静态变量的显式初始化赋值**和**静态代码块中的语句**，按源码出现顺序合并而成。如果类中没有需要执行的静态初始化逻辑，编译器不会生成该方法。

![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/AZiLbqEmkoGr8hxGwikc3N29n9c.jpeg)
...
![ce458ac9-1b63-4ad2-bc1a-55824c4206d1.png](images/WusgbQ4q9oqAZTxRaysc6v6qnae.png)
...
System.out.println(extensionClassLoader); // 输出：sun.misc.Launcher$ExtClassLoader@14ae5a5
...
问题：请问以上的类加载器存在继承关系吗？？？NO，调用getParent()方法，获取父类加载器（不是继承关系，而是一个属性关系）
...
在在Java应用程序开发中，类加载过程通常由三种内置的类加载器协同完成。在需要时，开发人员还可以自定义类加载器，以实现特定的类加载方式。

Java语言的一大亮点和强大之处，就在于允许开发者通过自定义类加载器来动态加载类库——无论是本地的JAR包还是网络上的资源。这种机制为实现灵活的插件系统提供了有力支持，使得应用程序能够在运行时动态扩展新功能。

此外，自定义类加载器还可用于实现应用隔离、字节码加密等高级场景。自定义类加载器需继承自 `<font style="color:rgb(15, 17, 21);background-color:rgb(235, 238, 242);">ClassLoader</font>` 类，并且其父类加载器默认为系统类加载器。

**面试题：Class.forName()与 classLoader.loadClass()的区别？**
...
![b3eb8e20-6012-47f4-acbc-8377db5be71b.png](images/Jj8YbrxXboiCNmxbVeyc0QN3nif.png)

双亲委派机制的好处，避免类的俯冲加载，确保类的全局唯一性，同时保护程序安全，防止核心API被随意篡改。例如：如果自己写了一个java.lang.String类就会因为双亲委派机制不能被加载，不会破坏原生的String类的加载。

![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/IufcbdHAIoa5ONx4tB1cztRInIf.jpeg)
...
想要实现灵活而动态的去创建对象，则就可以通过反射机制来实现。例如：把需要创建对象的类名保存在配置文件中，通过读取配置文件拿到类名，然后根据类名来创建对象，这就必须使用反射机制来实现。

【示例】读取配置文件中的类名，然后再创建对象
...
classinfo.properties配置文件中的内容如下：
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/ULJIbgVQFoyfU0xINgtcL6VOnld.jpeg)
...
| public Class[] getInterfaces() | 获得类实现的接口，返回的是数组类型。 |
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/De2Ibrle1oZK2BxO588cYicbn0g.jpeg)
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/VRm6bZjmfoFiRlxAKvoc6pE7n6e.jpeg)
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/JARjbWpPpo8mi2x5jnBcAmy5nDb.jpeg)
...
public static void main(String[] args) {
    try {
        // 获得Tiger类的Class对象
        Class<?> clazz = Class.forName("com.jkweilai.reflection.Tiger");
        // 获得Tiger类中公开的的name属性
        Field nameField = clazz.getField("name");
        // 通过反射机制来实例化Tiger对象
        Object obj = clazz.newInstance();
        // 给name成员变量赋值
        nameField.set(obj, "jkweilai");
        // 获得name成员变量值
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/Qz4Pb55fWoS9BEx1LNGcKpkTnqe.jpeg)
...
操作权限不够的方法时（例如私有权限的方法性），则还必须调用Method类提供的setAccessible(boolean flag)方法，并且设置该方法的实参为true，这样才能够去调用该私有方法，否则就会抛出IllegalAccessException异常。
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/Hml9bs4zJokKu4xJHdAcaR1yneh.jpeg)
...
| **方法名** | **描述** |
| --- | --- |
| public Constructor getConstructor(Class... parameterTypes) | 获得公开权限的构造方法，参数为构造方法的形参类型 |
| public Constructor getDeclaredConstructor(Class... parameterTypes) | 获得任意权限的构造方法，参数为构造方法的形参类型 |
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/IdJpbeGR6o3eJtxrvrtcfg5pnXf.jpeg)

# 反射中获取泛型参数

## 反射中泛型的概述

Java泛型本质是编译期的类型检查，运行时通过类型擦除实现兼容性。虽然泛型信息在运行时大部分被移除，但JDK 1.5引入的**Signature属性**在字节码中保留了关键的**泛型签名**，使得反射可以获取父类、字段、方法等声明的泛型参数，这为框架开发提供了类型推断的基础能力。
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/OLwRbqv7joXKgOxZ7a8cFXW3nrd.jpeg)
...
![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/AzcnbT1QjofeVWxHcR3cwLujn0e.jpeg)
...
```plaintext
className=com.jkweilai.p8.reflection.Orange
```
...
![185f41ea-7741-4222-b514-0937be1ae065.png](images/UuIXbLmgxoGJ7Lx9veCcIqyfnPd.png)

接下来，我们修改classinfo.properties配置文件中的数据，将className设置为“com.jkweilai.p8.reflection.Apple”，则继续执行程序输出的结果如下：

![d4b5ad2c-1bc3-471b-a169-9a3dd435e9b3.png](images/U1febIz0LoR6rtxGE9Dc64LUnJg.png)

![329891e3-e369-4cda-abbd-deba0fb144c9.jpeg](images/T6MBbAPUvoRdGqxfdjocuhGnnDe.jpeg)
...
第二步：创建配置文件，并在配置文件中明确好类名和方法。

```plaintext
className=com.jkweilai.p7.reflection.Tiger
methodName=eat
```
...
![77770584-3e22-46c1-9594-72c66679c1fd.png](images/EuyVbS9YZob4nBxBSXwcPEGEncC.png)

接下来，我们修改classinfo.properties配置文件中的数据，将className设置为“com.jkweilai.p7.reflection.Student”，将methodName设置为“show”，则继续执行程序输出的结果如下：

![3fdb4878-dd43-4185-a115-9c7cbf4e0c30.png](images/R0ulbOFY1o7bZvxY6Sgczdfxngd.png)