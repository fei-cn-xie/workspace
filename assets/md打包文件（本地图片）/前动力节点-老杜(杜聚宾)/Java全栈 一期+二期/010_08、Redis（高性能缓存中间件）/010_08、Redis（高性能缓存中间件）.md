...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/JQdxbO4lfoHD4NxNL6LcfBx0n0g.jpeg)
...
*缓存型*：如 Redis、Memcached（专为高速缓存设计）。 **【注意：不要错误的认为NoSQL数据库都是缓存数据库，这是不对的！！！！】**

*持久化型*：如 RocksDB（支持长期存储）。

#### 文档数据库：如 MongoDB（存储结构化文档，一个文档就是一个 JSON，不是你想的 word/pdf）。

![e8799ef7-dcff-4684-b4ba-320cd1ccee4f.png](images/XYP7bcoOVo0CFJxthrkcq2tbnef.png)

#### 列族数据库：如 Cassandra、HBase（适合海量数据分析）。

传统的数据库都是行式存储：**以行为单位存储**

![f6145d99-2d23-4fc2-bfd4-0b9dd356f436.png](images/NVD0bJ3iPoneAsxJLcQcgVVFnyh.png)

**优点：**查询**单个用户**的全部信息极快，因为一次磁盘读取就能拿到整行。

**缺点：**如果想做一个分析，比如“**计算全国用户的平均年龄**”，数据库必须**扫描整张表**，把每一行（连同姓名、城市、职业等无关字段）都从磁盘读进内存，再挑出“年龄”这一列来计算。大量I/O被浪费在读取无关数据上。

**列族数据库：以列为单位存储**

![8ea92133-23eb-4ec7-9b49-7aaf7944f377.png](images/SZRrbkKvSobA1mxr8V4cKl5Rn4b.png)

**优点：**进行上面那个“**计算平均年龄**”的分析时，数据库**只需要读取**`**<font style="color:rgb(15, 17, 21);background-color:rgb(235, 238, 242);">年龄</font>**`**这一个列文件**，极大地减少了I/O，计算速度飙升。

**缺点：**查询“用户ID=1的全部信息”反而变慢，因为它需要从多个列文件中分别读取数据再拼装起来。

#### 图数据库：如 Neo4j（处理复杂关系网络）。

图数据库 = 用“节点”和“边”存储关系，直接映射现实世界的网络结构

![6b835f0f-dcdd-4d10-9a94-0a6fd33a638b.png](images/EfhvbBpEcoxsHkx54NSc6KUtn5q.png)

![ff85f423-7027-4d78-bc47-1688e092411e.png](images/KyTCbFtZBomV1jxulZHcUj9HnXe.png)
...
- **关系型数据库**：
  ...
  - 支持ACID确保数据可靠性
  - 通过标准的SQL语句完成数据的操作
  - **比较适合垂直扩展**：要提升性能，通过提高单机硬件的性能达到的扩展叫做垂直扩展。【**关系型数据库的水平扩展会严重损害其核心优势（ACID事务和复杂关联查询），导致架构复杂度飙升，往往得不偿失。**】
- **非关系型数据库**：
  - 数据存储灵活
...
![f6f018c3-7d4d-41af-8931-4cbd7df993a1.png](images/LXSBbWveqoyEZ7xKag2coYfpn0c.png)
...
![786ca3f4-a42e-4cdd-9eb5-e15944f9dcc5.png](images/J6uibrzWeoBpfWxiLNVcgkUJnQe.png)

### 排行榜（Leaderboard）

游戏排行榜、直播打赏榜、运动步数榜，这些排行榜，Redis 都可以提供高性能实现。

这是因为**Redis 底层有一个 Sorted Set（有序集合）数据结构，能在插入数据时就自动维护全局顺序，所以查询排名时无需临时计算。**

### 分布式锁（Distributed Lock）

`synchronized`是单机锁，作用范围是：**单进程内**，多个线程之间。

分布式锁：**解决的是跨进程、跨机器的资源竞争问题。分布式锁是一种在分布式系统中，让多个进程/机器能够互斥地访问共享资源的协调机制。**

**Redis 可以实现分布式锁。**防止重复提交、秒杀库存扣减，都需要用到分布式锁。

## Redis特点

1. Redis以key-value方式将数据存储在内存中（缓存中）。
2. Redis的value支持多种类型，例如：string(字符串)/hash(哈希表)/list(列表)/set(集合)/zset(有序集合)，不同的类型底层数据结构不同。
3. Redis的核心网络模型是单线程的，采用单线程设计主要是为了避免多线程的竞争条件和锁的开销，简化实现并保证原子性操作。Redis从 **6.0** 版本开始引入了**有限的多线程支持**，但**核心逻辑仍是单线程**：
...
7. Redis可以搭建集群，并且实现了主从（**master-slave**）同步。
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/LgBKbbp2OowlugxpG8WcsLkbnvc.jpeg)
...
**SSL/TLS就是给网络通信套上“加密信封”，让Redis的数据传输从“明信片”变成“密码信”，防止中间人偷看或篡改。**
...
（**不要采用这种方式**）如果不启用 Redis 的 `TLS/SSL`安全协议，直接这样编译：
...
执行安装命令（**仍然在根目录下**）：`make install`

Redis的命令自动安装到这个目录下了：`/usr/local/bin`

![94c89bfb-5d6b-441d-a66f-afc13118f9c7.png](images/O8nwbeFNWoS6iixh2locVP7BnA1.png)
...
- **redis-cli**：Redis 官方提供的 命令行客户端工具，用于与 Redis 服务器交互。
- redis-sentinel：Redis 官方提供的 高可用性（HA）解决方案，核心功能是管理 Redis 主从架构的自动故障转移（Failover）和监控，确保服务持续可用。
- **redis-server**：Redis 的核心程序，负责 启动和运行 Redis 数据库服务。
...
![cff2e20a-85cc-4f27-b813-c2b6281a126f.png](images/QLZobMkl0oz9k4xzBxCcXFXwnHe.png)

**可以看到Redis的默认端口号为：6379**
...
![1545040e-1c41-4b13-a71a-d32082abb704.png](images/DEq4bVw8cohN1Dx6EJOcqEKjnJe.png)

第二步：将`/etc/redis.conf`文件中的`daemonize no`修改为`daemonize yes`，保存退出。

![46b56637-86fe-4e24-b70b-a93970b8e820.png](images/L0KGboub1od5pnxNhmTcYUlCnhe.png)

第三步：切换到`/usr/local/bin`目录下，执行`redis-server /etc/redis.conf`命令

![5f18929d-c021-4e16-8a06-340b763a3a25.png](images/CnMzbKV1SoyAn7x3RhQcILlnn2d.png)

测试一下：使用 `redis-cli` 客户端工具登录并`ping`测试一下

![cb16b0bd-c941-4acd-ae14-834ea1524b24.png](images/ODavbZrWzoxyhlxmR08cm4gfnTf.png)
...
![e7732a0e-0363-475f-a524-27925f1e5a0c.png](images/JRGpbj4azojAzXxyO6ZckSXgn9b.png)

也可以通过修改这个数字来改变数据库的数量，修改为32，或者64等都可以，修改后需要重启redis服务便可生效。

1. 默认使用的数据库是：0
2. 切换数据库的命令：select ，例如：select 1

![4e2f80d7-31d0-4aac-9e49-6b8c1566c116.png](images/KJncbM2Tio1dJnx1TfTcW9DanWc.png)

1. **实际使用建议：集群模式**下不支持多数据库（仅 DB 0 可用），因此多数场景下建议使用 DB 0，或通过key的前缀区分数据（例如 `user:info`、`order:info`），而非依赖多数据库，如果需要更高隔离性，可考虑多个Redis实例，而非多DB。
...
![1e51e9fe-57aa-4b12-b79c-436ace69b69c.png](images/WTtJbIKNuoM2NMxK7rYc7v3mnGh.png)

或者这样做：

![fbef4fd9-f34f-43a9-8726-f55a1ae35b85.png](images/CG87bM7McoaM94xnRPLcdB2Vnud.png)

如果Redis设置了密码，则关闭时需要指定密码：`redis-cli -a password shutdown`

验证redis服务是否已经关闭：

![1bebfde5-3d66-43d8-ad1e-760be507ddc7.png](images/HBiZbCTk5oe6aaxDerPcqPP3nZg.png)
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/HXSybfVXcoP0ZQxMSA9cn2Ydnac.jpeg)
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/GYsAbjLu3o5LpoxlTlqcLEcRnlb.jpeg)
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/Szujb48x3oxW1uxuBaFcvAi1nog.jpeg)
...
| --- | --- |
| keys * | 获取所有key |
| move | 将当前库中数据移动到dbindex库中，如果dbindex库中已存在该key，则无法移动 |
| randomkey | 随机返回当前库中的一个key |
| type | 获取value的数据类型 |
| del | 删除key-value数据 |
| exists | 判断key是否存在，0表示不存在，1表示存在 |
| expire seconds | 设置key有效期，失效后会自动删除，单位秒 |
| pexpire milliseconds | 设置key有效期，失效后会自动删除，单位毫秒 |
| persist | 删除`过期时间` |
...
| **命令** | **说明** |
| --- | --- |
| set | 存数据，如果key已经存在，则value自动覆盖。 |
| get | 取数据。如果指定的key不存在，则返回结果`(nil)`，该结果表示无任何数据。当然，`删除`或`修改`类的操作如果返回结果是`(nil)`，则表示未执行任何操作（因为目标不存在）。 |
| getrange start end | 获取value的片段，start用来指定开始下标，end用来指定结束下标。 |
| getset | 返回旧的值，再设置新的值。 |
| mset k1 v1 k2 v2 k3 v3 | 批量存数据 |
| mget k1 k2 k3 | 批量取数据 |
| setnx | 存数据，区别是：如果key不存在则存储，key存在则放弃存储。 |
| setrange offset | 从指定下标`offset`位置开始替换（要求下标必须是 `>= 0` 的整数），假设`username=zhangsan`，执行`setrange username 5 si`，最后的执行结果是`zhangsan`变成了`zhangsin`。 |
| incr | **只支持整数字符串**，递增1，如果key不存在，第一次执行`incr`时，会自动执行`set key 0`，然后将值增长1。 |
| incrby increment | **只支持整数字符串**，以指定步长递增。 |
| decr | **只支持整数字符串**，递减1。 |
| decrby increment | **只支持整数字符串**，以指定步长递减。 |
| incrbyfloat increment | ****以指定步长递增，支持浮点数操作。 |
| append | 在原数据后面追加 |
| strlen | 获取数据的长度 |
| object encoding | Redis 的一个调试命令，用于查看指定 key 的底层存储编码（即 Redis 内部使用的数据结构类型）。它可以帮助开发者优化内存使用或排查性能问题。 |
...
注意：**红色字体**相关命令的第一个字母 `l`表示 `list`

| **命令** | **说明** |
| --- | --- |
| lpush element [element ...] | 左压入。添加操作。`lpush numbers 1 2 3` |
| rpush element [element ...] | 右压入。添加操作。`rpush numbers 4 5 6` |
| lrange start stop | 将列表中的数据读取出来，start指定起始下标，stop指定结束下标。0是起始下标，-1是最后一个元素的下标。读取操作，无删除操作。 |
| lpop [count] | 左弹出。删除操作。未指定`count`时删除`1`个。 |
| rpop [count] | 右弹出。删除操作。未指定`count`时删除`1`个。 |
| llen | 获取列表中数据的个数 |
| lrem count element | `count > 0`：从左侧开始搜索，移除与`element`相等的元素，移除数量为count `count < 0`：从右侧开始搜索，移除与`element`相等的元素，移除数量为count `count = 0`：移除列表中所有与`element`相等的元素 |
| lindex index | 获取指定索引位置的元素。只读取，不删除。0是第一个元素，-1是最后一个元素。 |
| lset index element | 修改指定索引处的值。0是第一个元素，-1是最后一个元素。 |
| ltrim start stop | 裁剪列表，只保留`start`和`stop`区间的数据，其它数据删除。0是第一个元素，-1可以表示最后一个元素。 |
| linsert before | after pivot element |
| rpoplpush source destination | 用于移除`source`列表的最后一个元素，并将移除的元素添加到`destination`列表。 rpoplpush：source列表右弹，destination列表左压。 |

## hash类型常用命令

| **命令** | **说明** |
| --- | --- |
| hset field value [field value ...] | 1. 作用：给哈希表的字段赋值。 2. 哈希表不存在时，新建哈希表，并将`field value...`添加到哈希表。 3. 哈希表存在时，并且`field`已存在时，`value`将覆盖。 |
| hget field | 通过`field`获取`value` |
| hmget field [field ...] | 通过多个`field`获取多个`value` |
| hgetall | 获取所有的`field value` |
| hexists field | 判断这样的`field`是否存在，1表示存在，0表示不存在。 |
| hsetnx field value | 当哈希表中不存在这个`field`，则添加，如果`field`存在，则不处理。 |
| hincrby field increment | 把`field`对应的`value`以`increment`步长增长。如果`value`不是一个integer会报错。 |
| hdel field [field ...] | 删除一个或多个`field` |
| hkeys | 取出所有的`field` |
| hvals | 取出所有的`value` |
| hlen | 获取`field-value`的个数 |

## set类型常用命令

| **命令** | **说明** |
| --- | --- |
| sadd member [member ...] | 向set集合中添加成员。重复的成员只能添加1个。 |
| smembers | 列出所有成员。 |
| srem member [member ...] | 删除set集合中的成员。 |
| sismember member | 判断set集合中是否存在这个成员，1表示存在，0表示不存在。 |
| scard | 获取set集合中成员个数 |
| srandmember [count] | 随机获取set集合中的`[count]`个成员 |
| spop | 从set集合中随机的弹出一个成员（会删除掉） |
...
| **命令** | **说明** |
| --- | --- |
| zadd score member [score member ...] | 1. 添加数据到有序集合 2. score必须是一个数字型的 3. score存储的是双精度浮点数，如果score是10这样的整数， 底层也会存储为双精度浮点数。 4. score决定成员的排序顺序 5. 可以一次添加多个 |
| zincrby increment member | 将`member`的`score`增长`increment` |
| zscore member | 获取`member`的`score` |
| **zrange start stop** | 从已排序的集合中获取`start`到`stop`之间的`member列表`， 0是第一个元素的下标，-1是最后一个元素的下标。 |
| **zrange start stop withscores** | 获取`member列表`时带上`score` |
| **zrangebyscore min max** | 根据`score`来获取成员，`min`指定最低`score`，`max`指定最高`score` |
| **zrangebyscore min max withscores** | 获取`member列表`时带上`score` |
| **zrangebyscore min max withscores limit offset count** | 支持分页 |
| **zrevrangebyscore max min withscores limit offset count** | 降序方式取 |
| zcard | 成员个数 |
| zcount min max | 获取`min score`和`max score`之间成员个数 |
| zrem member [member ...] | 删除一个或多个成员 |

# Java使用Jedis操作Redis

![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/TYffbfupSoYFI7xDpQwcJgxFnmg.jpeg)
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/GG6ebd9LOorJltxpS0TcM2VMnch.jpeg)
...
![9c5c832d-cfe2-4711-9daf-c55b4ac884f4.png](images/PCHGbbV3VogbUWxc7Q4cioVYnoh.png)
...
**安全系数：以上公式计算的是最小值。实际情况下需要考虑****安全系数**。**它是来应付突发情况的。**

- **Redis连接池安全系数建议1.2-1.5倍，数据库连接池建议2-3倍，因为数据库连接创建成本更高、需要更多缓冲。**
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/BxqrbWWAlozeMXxxQ0ncbyKFnnd.jpeg)
...
- 控制用户对特定键模式的访问，例如执行 `ACL SETUSER username ~order:* ~product:*`，用户只能访问以 "order:" 或 "product:" 开头的键
  - `**<font style="color:rgb(15, 17, 21);background-color:rgb(235, 238, 242);">~</font>**`** 符号表示"键模式匹配"的权限前缀**
...
1. **重启 Redis服务（修改配置文件一定要重启 redis 服务）之后用以下命令**：
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/U9Eub3tsMoDjA4xdWYKczXWvnJG.jpeg)
...
3. 然后 `java` 客户端内置了根证书（用它来验证营业执照真伪），jdk安装之后jdk的目录中就自带了根证书。【**JAVA_HOME\lib\security\cacerts**】
...
**第二步**：在Redis的配置文件中添加以下内容：**把配置文件中的 **`**<font style="color:#DF2A3F;">port 6379</font>**`**注释掉，然后添加以下配置**
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/KkvcbrxKgoIuqvxwHLUcb79OnYc.jpeg)
...
1. 启动 SpringBoot 应用
2. 使用curl 测试接口：
  - curl -X POST "[http://localhost:8080/api/redis/set"](http://localhost:8080/api/redis/set%22) -d "key=test&value=hello"
  - curl "[http://localhost:8080/api/redis/get/test](http://localhost:8080/api/redis/get/test)"
  - curl -X DELETE "[http://localhost:8080/api/redis/delete/test"](http://localhost:8080/api/redis/delete?key=test%22)
  - curl "[http://localhost:8080/api/redis/has/test"](http://localhost:8080/api/redis/hasKey?key=test%22)

# Redis的持久化

![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/FqSkbTmQUoIAAPxP0bQcmFyFn5b.jpeg)
...
![270c45ce-0cf9-4db4-8857-084956e74956.png](images/SbEBbxmX9olqg0xXm9HcsVwsndd.png)

设置RDB备份规则：

![36713991-fa99-4031-a4b1-a5195318fc2e.png](images/IUglbdYLVoqHJMxPecJco80pnpf.png)

以上英文翻译为：你可以通过取消以下行的注释来显式设置这些参数

设置文件名：

![f87323a9-c75e-404b-bd61-91c0fd488f57.png](images/WarKbXIvZoONkzxCYUGcMfhSnfe.png)

设置存储目录：

![ec370fe9-e952-4ab5-bded-2ff5d25556e8.png](images/K9KbbpyW4oSpp8xvj1LchfeUntf.png)

设置备份失败是否停止写入：默认是yes，表示RDB备份失败后，redis的set、hset、lpush等写操作将无法使用。但读取命令仍然可用。

![17980b5f-bed1-4f82-8de1-6be8abf77bfe.png](images/PGABbcKFCoZtSqxOtIoctsLDnbb.png)

设置是否压缩rdb文件：默认yes

![adf4734a-1641-4b65-8a7c-704b6c59dfa1.png](images/TvhKb4i8loOV77xaQ1lcx9Vvn4d.png)
...
![73c1904f-752e-4463-bf54-6537905b686b.png](images/JGSQbprqXoZry5xbO2Xc7tVynJb.png)
...
| **参数** | **推荐值** | **说明** |
| --- | --- | --- |
| `<font style="color:rgb(64, 64, 64);">save</font>` | `<font style="color:rgb(64, 64, 64);">save 900 1</font>` `<font style="color:rgb(64, 64, 64);">save 300 10</font>` `<font style="color:rgb(64, 64, 64);">save 60 10000</font>` | **多级备份规则**： + 15分钟1次修改 → 备份 + 5分钟10次修改 → 备份 + 1分钟1万次修改 → 备份 |
| `<font style="color:rgb(64, 64, 64);">stop-writes-on-bgsave-error</font>` | `<font style="color:rgb(64, 64, 64);">yes</font>` | **备份失败时拒绝写入**，避免数据不一致（需配合监控）。 |
| `<font style="color:rgb(64, 64, 64);">rdbcompression</font>` | `<font style="color:rgb(64, 64, 64);">yes</font>` | **启用压缩**（LZF算法），减少磁盘占用（CPU换空间）。 |
| `<font style="color:rgb(64, 64, 64);">rdbchecksum</font>` | `<font style="color:rgb(64, 64, 64);">yes</font>` | **启用CRC64校验**，防止损坏的RDB文件被加载。 |
| `<font style="color:rgb(64, 64, 64);">dbfilename</font>` | `<font style="color:rgb(64, 64, 64);">dump-${port}.rdb</font>` | **按端口命名文件**（多实例部署时避免冲突）。 |
| `<font style="color:rgb(64, 64, 64);">dir</font>` | `<font style="color:rgb(64, 64, 64);">/data/redis/rdb</font>` | **指定备份目录**： + 使用独立磁盘分区 + 避免与AOF日志混存 |
...
AOF（Append-Only File）以日志的形式来记录每个写操作，将Redis执行过的所有**写指令**记录下来，读操作不记录，只许以追加的方式写入aof文件。（AOF文件中记录的时候以普通文本形式记录，将所有的写的操作命令记录到日志文件中。）
...
- **混合持久化**：
  - **混合持久化**就是**用RDB格式存“老数据”快照（base.rdb），用AOF格式存“新变动”日志（incr.aof），重启时先读快照再补日志，实现又快又全的恢复**。
- **分段滚动更新**：
  - “分段”体现在将完整数据**拆成基础快照(base.rdb)和增量日志(incr.aof)两个文件**；
...
![8f14a7ee-5ffe-48f4-b17d-d65594bb5e78.png](images/ROItbYrrxor3KmxFaaec9N3enQf.png)
...
**在 Redis 7+ 中，RDB 和 AOF 同时开启时会并行工作，但 AOF 在数据恢复时优先级更高。推荐生产环境同时启用两者，利用 RDB 的快照优势和 AOF 的实时安全性，并通过 **`**<font style="color:#DF2A3F;">aof-use-rdb-preamble</font>**`** 进一步优化性能。**
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/LmYzbDs9rou6FgxBnoScW1iYnuh.jpeg)
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/NCGobAC8YoF7SExvldoc9fvYnOb.jpeg)

## 主从复制的理解

主从复制指将一个Redis服务器（主节点）的数据复制到其他Redis服务器（从节点）。主节点负责**写操作**，从节点同步主节点数据并支持**读操作**。
...
创建从节点配置文件。将 `/etc/redis.conf` 复制一份生成 `/etc/redis/6380.conf`，然后按照以下配置对`6380.conf`文件进行修改：（**一个一个修改**）
...
创建从节点配置文件。将 `/etc/redis.conf` 复制一份生成 `/etc/redis/6381.conf`，然后按照以下配置对`6381.conf`文件进行修改：（**一个一个修改**）
...
![3ae226ef-bdd5-4a3f-91b6-a05db48e7487.jpeg](images/RQuzbKPd6oou0Gx99bjcyxKonXf.jpeg)
...
**Bitmaps就是String的一种特殊用法——把每个字节的8个位分别用作布尔标记，这样内存效率极高，比如1亿用户在线状态只需约12MB。**
...
Bitmaps 本质上**就是一个超长的开关阵列，每个位置只能是 开(1) 或 关(0)。**

## HyperLogLog

HyperLogLog 的核心作用：用极小空间（~12KB）估算海量数据中不重复元素的个数，接受微小误差（~1%），换取巨大内存节省。

**HyperLogLog = 统计海量唯一值的"模糊计数器"，用固定12KB内存换0.81%误差，适合"只关心数量不关心细节"的场景。**

**HyperLogLog 的原理：通过观察"哈希值尾部连续0的最大长度"来估算数据规模——就像根据"最长连阴天"推断雨季长短。**

**HyperLogLog 经常用来进行 UV 统计。**

**UV 统计：UV = 独立访客数（Unique Visitors）,UV 就是"有多少个不同的人来过"，一个人来100次也只算1次。**

**还有一个 PV，PV = 页面浏览量（Page Views）**

**PV 就是"页面被打开的总次数"，刷新一次页面就+1。**

**PV = 10+5+3+1 = 19次（总访问量）**
...
**Stream 是 Redis 的消息队列，让你能像微信聊天一样收发消息记录。**
...
- **计算器**：能对这些坐标进行快速计算，比如“找出我周围5公里内所有的钉子”，并算出它们离我多远。

Redis的**Geospatial类型底层是使用ZSet实现的**。（不是一种全新的类型，是基于 zset 实现的语法糖。）