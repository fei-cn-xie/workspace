...
> **核心思路**：每个` Worker Agent` 被包装成一个 `@tool`，Supervisor 通过调用这些工具来完成任务委派。Supervisor 负责`意图识别`和`对话管理`，Worker 只负责专项任务。

```mermaid
graph TD
    User["👤 用户"]

    subgraph Supervisor["Supervisor Agent（主控）"]
        direction TB
        S1["🧠 意图识别：判断用户需要哪个 Worker"]
        S2["⚡ 短期记忆：Redis checkpointer（对话历史）"]
        S3["🗄️ 长期记忆：Milvus store（跨会话记忆）"]
    end

    subgraph Workers["Worker Agents（以工具形式调用）"]
        W1["call_inquiry_agent\n智慧问诊 Agent"]
        W2["call_report_agent\n报告解读 Agent"]
        W3["call_drug_agent\n药物 Agent"]
        W4["call_knowledge_agent\n知识问答 Agent"]
        W5["call_operation_agent\n运营数据 Agent"]
    end
...
sequenceDiagram
    ...
    U->>S: 1. 发送消息
    S->>L: 2. 分析意图，决定调用哪个 Worker 工具
    L->>T: 3. 生成 tool_call（包含 message 参数）
    T->>W: 4. 执行工具 → 调用对应 Worker Agent
    W-->>T: 5. 处理后返回字符串结果
    T-->>S: 6. ToolMessage 写回 Supervisor 的消息列表
    S->>L: 7. 基于 Worker 结果生成最终回复
    L-->>U: 8. 返回给用户
```

## **Worker Agent 实现：暂时模拟功能**
