...
![image.png](images/Q0S0bdEl0oJbw1xGQ3zcIYM6nAg.png)

数据均为纯文本数据，如下为节选内容示例。

![image.png](images/ZxwnbKXi4olNmcxcuDhcEZxJn9f.png)
...
![image.png](images/A2u9b5qwKo6EEDxOxLCcBbCvnGf.png)

1. 首先填写**项目名称**（必填），其他两项可留空，点击确认**创建项目**

![image.png](images/R24abMrQmoWeWyxu1VKcqRcmnWf.png)

1. 项目创建后会跳转到**项目设置**页面，打开**模型配置**，选择数据生成时需要调用的大模型 API 接口

![image.png](images/KOLXbZhvUolSy8xxqdtcIoghnjd.png)

1. 这里以 DeepSeek 模型为例，修改模型**提供商**和**模型名称**，填写 **API 密钥**，点击**保存**后将数据保存到本地，在右上角选择配置好的模型

![image.png](images/Z5ESbkLAsoAOPGxq8fJcKUMRnBh.png)

![image.png](images/Assgbhq2FoVEXRxBFlWcTE0qnSc.png)

1. 打开**任务配置**页面，设置文本分割长度为最小 500 字符，最大 1000 字符。在问题生成设置中，修改为每 10 个字符生成一个问题，修改后在页面最下方**保存任务配置**

![image.png](images/Tx0tbr86DoStG2xk9vdcNPhWnKe.png)

### 处理数据文件

1. 打开**文献处理**页面，选择并上传示例数据文件，选择文件后点击**上传并处理文件**

![image.png](images/QD1xbUj1HoiBe9xbI9Mcm01Xn3d.png)

![image.png](images/DpU1bx9TOoFyqZxpsd1cVb9Nngg.png)

1. 上传后会调用大模型解析文件内容并分块，耐心等待文件处理完成，示例数据通常需要 2 分钟左右

![image.png](images/YIgEbBzbBovIYBxMdp4cd0o3nce.png)

### 生成微调数据

1. 待文件处理结束后，可以看到文本分割后的文本段，选择全部文本段，点击**批量生成问题**

![image.png](images/FiBmbLuPCobFaBxLyIQc3YeRn9c.png)

1. 点击后会调用大模型根据文本块来构建问题，耐心等待处理完成。视 API 速度，处理时间可能在 20-40 分钟不等

![image.png](images/FQp6bXsZgo8a4NxgvdxcNtvVn1c.png)

1. 处理完成后，打开**问题管理**页面，选择全部问题，点击**批量构造数据集**，耐心等待数据生成。视 API 速度，处理时间可能在 20-40 分钟不等

![image.png](images/Hz8PbyJAKoOVHHxeE07c2HuUnhd.png)

![image.png](images/H0Syb4Aq9orA7fxkO94cwdPunMh.png)
...
![image.png](images/Yf3MbiaoEotGC7xX4LzcGpY1nQc.png)

1. 在导出配置中选择**在 LLaMA Factory 中使用**，点击**更新 LLaMA Factory 配置**，即可在对应文件夹下生成配置文件，点击**复制**按钮可以将配置路径复制到粘贴板。

![image.png](images/Eu7cb2Rxmo2CmQxJuidc0dfinie.png)
...
![image.png](images/XOavbDi3loWrktxvOK1cvNhinDg.png)