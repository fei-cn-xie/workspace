...
![image.png](images/HWHzbGnxCoOzIOxgPG0c0fhEnCe.png)
...
![image.png](images/Mo1YbKEfWonwMdxPX7ecOenlnke.png)

1. 进入 Web UI 界面后，选择模型为 Qwen2.5-3B-Instruct，模型路径可填写本地绝对路径，不填则从互联网下载

![image.png](images/W26nbkMJloNSHKxwlQzcunWinNg.png)

1. 将**数据路径**改为使用 Easy Dataset 导出的配置路径，选择 Alpaca 格式数据集

![image.png](images/GDyjbiMOXogcl1xccubcg5MTnwh.png)

1. 为了让模型更好地学习数据知识，将**学习率**改为 1e-4，**训练轮数**提高到 8 轮。批处理大小和梯度累计则根据设备显存大小调整，在显存允许的情况下提高批处理大小有助于加速训练，一般保持批处理大小×梯度累积×显卡数量等于 32 即可

![image.png](images/XlLlbewTIodRlgxKDMUcDc4InIg.png)

1. 点击其他参数设置，将**保存间隔**设置为 50，保存更多的检查点，有助于观察模型效果随训练轮数的变化

![image.png](images/SqbRbqfzcoud6lx9G0dcAjaHnFh.png)

1. 点击 LoRA 参数设置，将 **LoRA 秩**设置为 16，并把 **LoRA 缩放系数**设置为 32

![image.png](images/GSqTbz5aFoCiwrxTNRPcyDW6nod.png)

1. 点击**开始**按钮，等待模型下载，一段时间后应能观察到训练过程的损失曲线

![image.png](images/GQrTb8l3Ioc0vmxOsuNcsskfnDs.png)

1. 等待模型训练完毕，视显卡性能，训练时间可能在 20-60 分钟不等

![image.png](images/GFPzby2MuoxQgExKZvPc213EnMe.png)

## 验证微调效果

1. 选择**检查点路径**为刚才的输出目录，打开 **Chat** 页面，点击**加载模型**

![image.png](images/LXaybWBfboQL6sxaonUc5KQIn34.png)

1. 在下方的对话框中输入问题后，点击提交与模型进行对话，经与原始数据比对发现微调后的模型回答正确

![image.png](images/FOeUbPiZZoYaaYxzqcqch7J5njh.png)

1. 点击**卸载模型**将微调后的模型卸载，清空**检查点路径**，点击**加载模型**加载微调前的原始模型

![image.png](images/Wo9VbWYQpoRootx5shWcgofUnO6.png)

1. 输入相同的问题与模型进行对话，发现原始模型回答错误，证明微调有效

![image.png](images/HiMzbPtdSobIU2xP6xWchRJ0n6g.png)
